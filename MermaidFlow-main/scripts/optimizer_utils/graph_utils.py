...
from scripts.prompts.optimize_prompt import (
    ...
    WORKFLOW_TEMPLATE_WITH_MERMAID,
    WORKFLOW_TEMPLATE,
)
...
class GraphUtils:
    ...
    def read_mermaid_files(self, round_number: int, workflows_path: str):
        mermaid_file_path = os.path.join(workflows_path, f"round_{round_number}", "graph.mmd")
        ...
        try:
            ...
            logger.error(f"Error: Mermaid file not found for round {round_number}: {e}")
            # Return a default empty graph if file not found
            return "flowchart TD\nA[No graph available]"
        except Exception as e:
            logger.error(f"Error loading mermaid for round {round_number}: {e}")
            raise
    ...
    def write_graph_files_with_mermaid(self, directory: str, response: dict, round_number: int, dataset: str):
        code = WORKFLOW_TEMPLATE_WITH_MERMAID.format(code=response["code"], round=round_number, dataset=dataset)
        ...
        with open(os.path.join(directory, "graph.mmd"), "w", encoding="utf-8") as file:
            file.write(response["graph"])
